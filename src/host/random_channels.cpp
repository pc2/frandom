...
#endif
#define ADDRESS_CALC_KERNEL "addressCalculationMemory"
...
void calculate(cl::Context context, cl::Device device, cl::Program program){
	...
	cl::CommandQueue address_queue(context, device);  
	#pragma PY_CODE_GEN block_start
    cl::CommandQueue $TYPE_queue$N(context, device);
	#pragma PY_CODE_GEN block_end [CODE.replace("$N", str(i)).replace("$TYPE", t) for i in range(replications) for t in ["read", "write"]]

    //Create Buffers for input and output
	cl::Buffer Buffer_data(context, CL_MEM_READ_ONLY, sizeof(DATA_TYPE)*DATA_LENGTH);
	cl::Buffer Buffer_random(context, CL_MEM_WRITE_ONLY, sizeof(DATA_TYPE_UNSIGNED) * RANDOM_LENGTH);

    // create the kernels
    cl::Kernel addresskernel(program, ADDRESS_CALC_KERNEL, &err);
    assert(err==CL_SUCCESS);

	//prepare kernels
	err = addresskernel.setArg(0, Buffer_random);
	assert(err==CL_SUCCESS);
	err = addresskernel.setArg(1, RANDOM_LENGTH);
	assert(err==CL_SUCCESS);

	#pragma PY_CODE_GEN block_start
    cl::Kernel $TYPEkernel$N(program, "$TYPEMemory$N", &err);
    assert(err==CL_SUCCESS);

	//prepare kernels
    err = $TYPEkernel$N.setArg(0, Buffer_data);
    assert(err==CL_SUCCESS);
    err = $TYPEkernel$N.setArg(1, DATA_LENGTH);
    assert(err==CL_SUCCESS);
	...
	address_queue.enqueueWriteBuffer(Buffer_random, CL_TRUE, 0, sizeof(DATA_TYPE_UNSIGNED) * RANDOM_LENGTH, random);

    double t;
    t = mysecond();
    for (int i = 0; i < NTIMES; i++){
        t = mysecond();
        address_queue.enqueueTask(accesskernel);
		#pragma PY_CODE_GEN block_start
        $TYPE_queue$N.enqueueTask($TYPEkernel$N);
		#pragma PY_CODE_GEN block_end [CODE.replace("$N", str(i)).replace("$TYPE", t) for i in range(replications) for t in ["read", "write"]]
		#pragma PY_CODE_GEN block_start
        $TYPE_queue$N.finish();
		#pragma PY_CODE_GEN block_end [CODE.replace("$N", str(i)).replace("$TYPE", t) for i in range(replications) for t in ["read", "write"]]
        address_queue.finish();	
        t = mysecond() - t;
        timearray[i] = t;
    }

    address_queue.enqueueReadBuffer(Buffer_data, CL_TRUE, 0, sizeof(DATA_TYPE)*DATA_LENGTH, data);
    err=address_queue.finish();
