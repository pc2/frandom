...
void calculate(cl::Context context, cl::Device device, cl::Program program){
	...
	assert(err==CL_SUCCESS);
	#pragma PY_CODE_GEN block_end [replace(replace_dict={**globals(), **locals(), "repp1": (rep % 4 ) + 1}) for rep in range(replications)] 
    double t;
    double tmin = DBL_MAX;
    t = mysecond();
    for (int i = 0; i < NTIMES; i++){
        ...
        t = mysecond();
		#pragma PY_CODE_GEN block_start
        compute_queue$rep$.enqueueNDRangeKernel(accesskernel$rep$, cl::NDRange(1), cl::NDRange(128), cl::NDRange(1), NULL, NULL);
		#pragma PY_CODE_GEN block_end [replace(replace_dict=locals()) for rep in range(replications)] 
		#pragma PY_CODE_GEN block_start
		compute_queue$rep$.finish();
		#pragma PY_CODE_GEN block_end [replace(replace_dict=locals()) for rep in range(replications)]
